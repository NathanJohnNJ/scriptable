...
class Configuration {
    ...
    requestTimeoutInterval = 3;
    pluginsOrSwUpdatesToIgnore = []; 
    adaptToLightOrDarkMode = true;
    bgColorMode = 'CUSTOM';
    customBackgroundColor1_light = '#8F713C'; 
    customBackgroundColor2_light = '#AD9A5C'; 
    customBackgroundColor1_dark = '#001039'; 
    customBackgroundColor2_dark = '#1B5089';
    chartColor_light = '#001E4B';
    chartColor_dark = '#AB9152';
    fontColor_light = '#0A0A0C'; 
    fontColor_dark = '#A4A8AD';
    failIcon = '❌';
    bulletPointIcon = '▫️';
    decimalChar = '.';
    jsonVersion = 3;
    ...
    siriGui_icon_version_color = '#306A9D';
...
const maxLineWidth = 355; 
const normalLineHeight = 42;
...
async function initializeFileManager_Configuration_TimeFormatter_Fonts_AndToken() {
    ...
    if (usePersistedConfiguration && !overwritePersistedConfig) {
        CONFIGURATION = await getPersistedObject(getFilePath(configurationFileName), 3, CONFIGURATION, false);
...
async function buildUsualGui(widget, homeBridgeStatus) {
    ...
    if (cpuData && ramData) {
        ...
        widget.addSpacer(10);
        let updatedAt = addStyledText(widget, 'Updated: ' + timeFormatter.string(new Date()), updatedAtFont);
...
async function handleNotifications(hbRunning, hbUtd, pluginsUtd, nodeUtd) {
    ...
    let state = await getPersistedObject(path, 1, INITIAL_NOTIFICATION_STATE, true);
    let now = new Date();
