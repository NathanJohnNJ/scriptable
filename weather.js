const apiKey = "31c0b0b062e2eb7d13664b499cbeb8e4"
...
const textFormat = {
  ...
  largeTemp:   { size: 34, color: "ffffff", font: "light" },
  smallTemp:   { size: 14, color: "ffffff", font: "" },
  tinyTemp:    { size: 12, color: "ffffff", font: "" },
  
  customText:  { size: 10, color: "ffffff", font: "light" },
  
  sunrise:     { size: 14, color: "ffffff", font: "medium" },

  location:     { size: 16, color: "ffffff", font: "bold", opacity: 1},
  desconly:  { size: 12, color: "ffffff", font: "regular", opacity: 1},
  HiLotemp:     { size: 12, color: "ffffff", font: "regular", opacity: 1},
  databtm:     { size: 10, color: "ffffff", font: "bold", opacity: 1},
  databtmtxt:     { size: 10, color: "ffffff", font: "regular", opacity: 1},
  updatetxt:     { size: 11, color: "ffffff", font: "semibold", opacity: 1},
  warning:     { size: 12, color: "ffffff", font: "bold" },
  alertstxt:     { size: 10, color: "ffffff", font: "bold", opacity: 1},
}
...
bgurl = bgSettings.url()
widget.backgroundImage = await getImage(bgurl)
...
function provideFunction(name) {
  const functions = {
    ...
    cloudspic() { return cloudspic },
    sunrisepic() { return sunrisepic },
    sunsetpic() { return sunsetpic },
    rainpic() { return rainpic },
...
async function background() {
  ...
  let background = {
    dawn: {
      url() { return 'https://i.imgur.com/IkphWmC.png' },
    },
    sunrise: {
      url() { return 'https://i.imgur.com/IkphWmC.png' },
    },
    midday: {
      url() { return 'https://i.imgur.com/N6XnDmv.png' },
    },
    noon: {
      url() { return 'https://i.imgur.com/N6XnDmv.png' },
    },
    sunset: {
      url() { return 'https://i.imgur.com/0idT9qW.png' },
    },
    twilight: {
      url() { return 'https://i.imgur.com/0idT9qW.png' },
    },
    night: {
      url() { return 'https://i.imgur.com/jw2YiXx.png' },
    },
  }
  const sunrise = sunData.sunrise
  const sunset = sunData.sunset
  if (closeTo(sunrise)<=15) { return background.sunrise }
  if (closeTo(sunset)<=15) { return background.sunset }
  if (closeTo(sunrise)<=45 && currentDate.getTime() < sunrise) { return background.dawn }
  if (closeTo(sunset)<=45 && currentDate.getTime() > sunset) { return background.twilight }
  if (isNight(currentDate)) { return background.night }
  if (currentDate.getHours() == 12) { return background.noon }
  return background.midday
}
...
async function drawdiagram(column) {
  ...
  diagramStack.borderWidth = 0
	drawLine(0,15,665,15, 3, linecolor)
	// DRAW THE GRAPH
  let min, max, diff;
  ...
  let heightDiff = 80
	...
	// endo of for-loop
	drawLine(0,255,665,255, 3, linecolor)
...
async function desconly(column) {
  ...
  const str2 = descaloneText.charAt(0).toUpperCase() + descaloneText.slice(1);
	const descalone = provideText(str2, descaloneStack, textFormat.desconly)
}
...
console.log("direction " + direction)
  ...
  if (direction < 326.25) winddir = "NW";       if (direction < 303.75) winddir = "WNW";
...
async function updatedtime(column) {
  ...
  const updatedText = ("Updated at " + weatherData.update)
const updatevar = provideText(updatedText, updatedStack, textFormat.updatetxt)
async function rainpic(column) {
  let rainStack = align(column)
  rainStack.setPadding(1, 0, 0, 0)
  const rain = provideText('☔️', rainStack, textFormat.defaultText)
}

  //const updatevar = provideText(updatedText, updatedStack, textFormat.updated)

}
async function cloudspic(column) {
  let cloudStack = align(column)
  cloudStack.setPadding(1, 0, 0, 0)
  const clouds = provideText('☁️', cloudStack, textFormat.defaultText)
}
async function sunrisepic(column) {
  if (!sunData) { await setupSunrise() }
  const sunriseStack = align(column)
  sunriseStack.setPadding(1, 0, 0, 0)
	const sunrisepic = provideText('🌅', sunriseStack, textFormat.defaultText)
  return sunrisepic
}

async function sunsetpic(column) {
  let sunsetStack = align(column)
  sunsetStack.setPadding(1, 0, 0, 0)
  const sunsetpic = provideText('🌄', sunsetStack, textFormat.defaultText)
	return sunsetpic
}
async function rainpic(column) {
  let rainStack = align(column)
  rainStack.setPadding(1, 0, 0, 0)
  const rainpic = provideText('☔️', rainStack, textFormat.defaultText)
	return rainpic
}
